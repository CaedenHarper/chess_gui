...
inline std::string evalToString(const int eval) noexcept {
    if(isMate(eval)) {
        ...
        return eval > 0 ? "#" + std::to_string(movesToMate) : "#-" + std::to_string(movesToMate);
    }
...
public:
    ...
    static constexpr int pieceValueFromType(const Piece piece) {
        switch(piece.type()) {
            case PieceType::Pawn:
                return Eval::PAWN_COST;
            case PieceType::Knight:
                return Eval::KNIGHT_COST;
            case PieceType::Bishop:
                return Eval::BISHOP_COST;
            case PieceType::Rook:
                return Eval::ROOK_COST;
            case PieceType::Queen:
                return Eval::QUEEN_COST;
            default:
                return 0; // king and empty square
        }
    }
