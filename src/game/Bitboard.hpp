...
public:
    ...
    constexpr bool intersects(const Bitboard& other) const {
        return (bitboard_ & other.bitboard_) != 0;
    }
    // If bitboard has the given bit.
    constexpr bool hasBit(uint64_t bit) const {
        return (bitboard_ & bit) != 0;
    }
    // If bitboard contains the given chess square.
    constexpr bool containsSquare(int square) const {
        return (bitboard_ & (1ULL << square)) != 0;
    }
    // Is bitboard empty.
    ...
    constexpr void setSquare(int square) {
        bitboard_ |= (1ULL << square);
    }
    // Set given bit.
    constexpr void setBit(uint64_t bit) {
        bitboard_ |= bit;
    }

    // Clear given chess square.
    constexpr void clearSquare(int square) {
        bitboard_ &= ~(1ULL << square);
    }
    // Clear given bit.
    constexpr void clearBit(uint64_t bit) {
        bitboard_ &= ~bit;
    }
    ...
    constexpr Bitboard mask(const Bitboard& other) const {
        return Bitboard{bitboard_ & other.bitboard_};
    }

    // Merge given bitboard and create a new bitboard, without updating this bitboard.
    constexpr Bitboard merge(const Bitboard& other) const {
        return Bitboard(bitboard_ | other.bitboard_);
    }
    // Set this bitboard to this bitboard merged with another.
    constexpr void mergeIn(const Bitboard& other) {
        bitboard_ |= other.bitboard_;
    }

    // Return bitboard with 0's to 1's and 1's to 0's flipped; return bitwise negation of the bitboard.
    constexpr Bitboard flip() const {
        return Bitboard{~bitboard_};
    }

    // Shift to the right by `numBits` bits
    constexpr Bitboard rightShift(uint64_t numBits) const {
        return Bitboard{bitboard_ >> numBits};
    }
    // Shift to the left by `numBits` bits
    constexpr Bitboard leftShift(uint64_t numBits) const {
        return Bitboard{bitboard_ << numBits};
    }

    constexpr int lsbIndex() const {
        return __builtin_ctzll(bitboard_);
    }
    constexpr int msbIndex() const {
        return (Utils::NUM_SQUARES - 1) - __builtin_clzll(bitboard_);
    }
    ...
    // Get bit from square.
    static constexpr uint64_t bit(int square) {
        return 1ULL << square;
    }

    // Get string representation of bitboard.
    std::string to_string() const {
        return std::bitset<Utils::NUM_SQUARES>(raw()).to_string();
    }
