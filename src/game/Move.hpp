...
enum class Promotion : uint8_t { None, Knight, Bishop, Rook, Queen };
...
class Move {
public:
    // Create a junk Move. Note packed_ will hold undefined data. This is needed so MoveList is faster (instead of
    // initializing packed to 0).
    ...
    // Create a move using Pieces for context for flag + promotion. This is slower, only use if necessary / not in hot
    // loop.
    ...
    constexpr uint8_t sourceSquare() const noexcept {
        return (packed_ >> SOURCE_SHIFT) & SOURCE_MASK;
    }
    constexpr uint8_t targetSquare() const noexcept {
        return (packed_ >> TARGET_SHIFT) & TARGET_MASK;
    }
    constexpr MoveFlag flag() const noexcept {
        return static_cast<MoveFlag>((packed_ >> FLAG_SHIFT) & FLAG_MASK);
    }
    constexpr Promotion promotion() const noexcept {
        return static_cast<Promotion>((packed_ >> PROMO_SHIFT) & PROMO_MASK);
    }
    ...
    constexpr bool isDoublePawn() const noexcept {
        return flag() == MoveFlag::DoublePawnPush;
    }
    constexpr bool isKingSideCastle() const noexcept {
        return flag() == MoveFlag::KingCastle;
    }
    constexpr bool isQueenSideCastle() const noexcept {
    ...
    static constexpr PieceType promotionToPieceType(Promotion promotion) noexcept {
        switch(promotion) {
            ...
            default:
                assert(false);
                return PieceType::None; // should never happen
        }
    }
...
struct MoveList { // NOLINT(cppcoreguidelines-pro-type-member-init, hicpp-member-init) It's okay that data is junk data
                  // here, it heavily improves performance
